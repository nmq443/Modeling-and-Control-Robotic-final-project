...
[X,Y,Z] = sphere; % generate sphere uses N = 20, generates 3 (N+1)-by-(N+1) matrices so that 
% SURF(X,Y,Z) produces a unit sphere
R = 0.15;
X = R*X;
Y = R*Y;
Z = R*Z;
...
for i=1:size(x_dot,2)
    q = robot.GetState();
    [qd, q_control_dot] = cartesianSpaceController(x(:,i), x_dot(:,i), q, Ts, Kp, obs(:,1), k0);
